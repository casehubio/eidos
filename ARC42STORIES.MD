...
- **VocabularyRegistry / CdiVocabularyRegistry** — pluggable vocabulary system; CDI `Instance<VocabularyRegistrar>` discovery with cross-vocabulary equivalence traversal and XKOS-style subsumption hierarchy (`specializes()` → global DAG across vocabulary boundaries; `subsumes()`, `match()` → `MatchDegree`, `ancestors()`, `descendants()`, `expandForMatchingByVocabulary()`)
- **CapabilityHealth / ReactiveCapabilityHealth** — two-layer probe: declared (descriptor) vs. operable now (runtime degradation + behavioral compliance); `AgentStateStore` SPI records degradation with TTL; `BehavioralSignalStore` SPI accumulates learned signals (DECLINE, SUCCESS, COMPLIANT, VIOLATED) with per-signal TTL; probe steps: Degraded → Unavailable → Excluded(DECLARED) → Excluded(LEARNED) → EpistemicallyWeak → BehavioralViolation → Ready; aggregate violation safety valve detects cross-dimensional drift
...
| L2 CDI Discovery | `CdiVocabularyRegistry` — `Instance<VocabularyRegistrar>` discovery; XKOS-style subsumption hierarchy (global DAG, cross-vocabulary specialization); in-memory registry alternatives | C1 |
| L3 JPA Registry | `JpaAgentRegistry`, Flyway schema (`agent_descriptor`, `agent_capability`); reactive build-gated variant | C1, C3 |
| L4 Capability Health | `DefaultCapabilityHealth`; `AgentStateStore` degradation tracking; `BehavioralSignalStore` compliance signals; fail-fast 6-step probe (Degraded → Unavailable → Excluded → EpistemicallyWeak → BehavioralViolation → Ready); aggregate violation safety valve | C2, C3 |
...
C4Container
  ...
  Container_Boundary(runtime_b, "L2–L5 Runtime — casehub-eidos") {
    Container(vocab_cdi, "CdiVocabularyRegistry", "CDI @ApplicationScoped", "Discovers VocabularyRegistrar beans via Instance<VocabularyRegistrar>; two-pass registration; global subsumption DAG with cross-vocabulary specialization; per-vocabulary index injection")
...
sequenceDiagram
  ...
  DB-->>JPA: rows
  JPA-->>App: List<AgentMatch> (ordered by match quality when capability is queried)
```
...
sequenceDiagram
  ...
  participant BS as BehavioralSignalStore
  ...
  alt Step 1: DegradationReason present
    CH-->>Eng: Degraded(reason, message)
  else Step 2: capability not declared
    CH-->>Eng: Unavailable("code-review not declared")
  else Step 3: excludedDomains contains taskDomain
    CH-->>Eng: Excluded(domain, DECLARED, 0)
  else Step 4: learned DECLINE count ≥ threshold
    CH-->>Eng: Excluded(domain, LEARNED, count)
  else Step 5: epistemicDomains[domain] < threshold
    CH-->>Eng: EpistemicallyWeak("rust", 0.2)
  else Step 6: behavioral compliance
    CH->>BS: learned(agentId, tenancyId, capability, VIOLATED)
    BS-->>CH: Map<dimension, count>
    alt per-dimension count ≥ threshold
      CH-->>Eng: BehavioralViolation(exceeding, PER_DIMENSION)
    else aggregate total ≥ threshold
      CH-->>Eng: BehavioralViolation(all, AGGREGATE)
    else
      CH-->>Eng: Ready
    end
  end
...
Probe order is fail-fast: degradation state first (known-bad agents are not worth checking further), then capability resolution, then declared exclusions, then learned exclusions (DECLINE signals), then epistemic confidence, then behavioral compliance (VIOLATED signals — per-dimension then aggregate).
...
**Issues:** eidos#1, #4, #5, #15, #20, #22, #32, #36, #37, #71, #73
**Navigation:** `git log --grep="#1\|#4\|#5\|#15\|#20\|#22\|#32\|#71\|#73" --oneline api/`
...
- **Capability with epistemic domain confidence** — `AgentCapability(name, qualityHint, latencyHintP50Ms, epistemicDomains Map<String,Double>, excludedDomains Set<String>)` qualifies declared capability by domain confidence and explicit negative specialization (e.g. `epistemicDomains: {"java": 0.95}`, `excludedDomains: {"rust"}`)
...
- **`MatchDegree` sealed interface** — `Exact`, `Plugin(depth)`, `Specialization(depth)`, `None` — returned by `VocabularyRegistry.match()` to express how well a requested vocabulary term matches a declared term; `Plugin` means the declared term is more specific (depth = hierarchy steps from declared down to requested); `Specialization` means the declared term is more general (depth = steps from requested up to declared)
- **`VocabularyTerm.specializes()`** — `default List<VocabularyTerm> specializes()` declares XKOS-style parent terms; enables subsumption hierarchy construction across vocabulary boundaries (application terms can specialize foundation terms); consumed by `CdiVocabularyRegistry` during two-pass registration to build global DAG
- **`VocabularyRegistry` subsumption API** — `subsumes(vocabUri, general, specific)` tests hierarchy containment; `match(vocabUri, declared, requested)` → `MatchDegree`; `ancestors(vocabUri, value)` and `descendants(vocabUri, value)` traverse the DAG; `expandForMatchingByVocabulary(value)` groups all terms matching a value (including cross-vocabulary specialization edges) by declaring vocabulary URI
- **Sealed CapabilityStatus** — `Ready` / `Unavailable` / `Degraded(reason, msg)` / `EpistemicallyWeak(domain, confidence)` / `Excluded(domain, ExclusionSource, declineCount)` / `BehavioralViolation(violations, ViolationKind)` — exhaustive probe result; callers must handle all six in a switch; `ViolationKind { PER_DIMENSION, AGGREGATE }` discriminates single-dimension spike from cross-dimensional drift; probe order: Degraded → Unavailable → Excluded(DECLARED) → Excluded(LEARNED) → EpistemicallyWeak → BehavioralViolation → Ready
...
- `api/src/main/java/io/casehub/eidos/api/AgentCapability.java` — capability record with `qualityHint` (Double), `epistemicDomains` (Map), `excludedDomains` (Set<String>, overlap invariant with epistemicDomains), and validated string fields; Builder inner class
- `api/src/main/java/io/casehub/eidos/api/BehavioralSignal.java` — enum: `DECLINE`, `SUCCESS`, `COMPLIANT`, `VIOLATED` — discriminator for `BehavioralSignalStore` methods; COMPLIANT/VIOLATED pair supports compliance observation recording
- `api/src/main/java/io/casehub/eidos/api/BehavioralSignalStore.java` — SPI: `record`, `clear`, `learned`, `count` — signal-parameterized via `BehavioralSignal`; qualifier parameter (task domain for DECLINE/SUCCESS, `ComplianceDimension` key for COMPLIANT/VIOLATED); per-signal TTL owned by store implementation
- `api/src/main/java/io/casehub/eidos/api/ComplianceDimension.java` — constants: `LATENCY`, `ATTESTATION_RATE`, `DELEGATION`, `ESCALATION` dimension keys; `ATTESTOR_ID`, `TRUST_DIMENSION_PREFIX`, `LATENCY_VIOLATION_MULTIPLIER` conventions
- `api/src/main/java/io/casehub/eidos/api/BehavioralExpectations.java` — static utility: `latencyBound(AgentCapability)` → `OptionalLong`, `delegationExpected(AgentDisposition)` → `boolean`, `escalationExpected(AgentDisposition, String, VocabularyRegistry)` → `boolean`; extracts testable expectations from descriptor fields
- `api/src/main/java/io/casehub/eidos/api/CapabilityResolver.java` — static utility: `resolve(capabilities, tag, registry)` → best-matching `AgentCapability`; `match(capability, tag, registry)` → `MatchDegree`; shared by probe and recording paths
- `api/src/main/java/io/casehub/eidos/api/MatchDegree.java` — sealed interface: `Exact`, `Plugin(depth)`, `Specialization(depth)`, `None`; returned by `VocabularyRegistry.match()`; depth counts hierarchy steps between declared and requested terms
...
**Issues:** eidos#1, #3, #71, #73
**Navigation:** `git log --grep="#1\|#3\|#71\|#73" --oneline runtime/src/main/java/io/casehub/eidos/runtime/vocabulary/`
...
**After:** `CdiVocabularyRegistry @ApplicationScoped` — discovers `Instance<VocabularyRegistrar>` CDI beans at startup; two-pass registration (term registration → hierarchy construction); `equivalentValues` traverses `exactMatches` links bidirectionally; XKOS-style subsumption hierarchy via `specializes()` builds a global DAG across vocabulary boundaries with `subsumes()`, `match()` → `MatchDegree`, `ancestors()`, `descendants()`.

What this layer adds:

- **Pluggable vocabulary via CDI** — domain apps provide `VocabularyRegistrar` implementations as `@ApplicationScoped` CDI beans; `CdiVocabularyRegistry` discovers them via `@Any Instance<VocabularyRegistrar>` at `@PostConstruct`
...
- **XKOS-style subsumption hierarchy** — `VocabularyTerm.specializes()` declares parent terms; `CdiVocabularyRegistry` builds a global DAG with ancestor/descendant indexes during two-pass registration; `subsumes(vocabUri, general, specific)` tests containment; `match(vocabUri, declared, requested)` returns `MatchDegree` (Exact, Plugin, Specialization, None)
- **Cross-vocabulary specialization** — application-tier vocabulary terms can specialize foundation-tier terms across vocabulary boundaries (e.g. `CasehubCapabilityTerm.JAVA_CODE_REVIEW` specializes `CasehubCapabilityTerm.CODE_REVIEW`); per-vocabulary index injection enables bidirectional `match()` across vocabularies with inline collision detection (native-vs-injected, injected-vs-injected)
- **Runtime registration** — `VocabularyRegistry.register(Class<T>)` supports programmatic registration with late-register atomicity (snapshot-rollback on failure)
...
| No hierarchy-aware matching | `find(capabilityName)` requires exact string equality; "java-code-review" does not match agents declaring "code-review" | `VocabularyTerm.specializes()` → global DAG; `match()` returns `MatchDegree.Plugin(depth)` or `Specialization(depth)` |
| Cross-vocabulary specialization impossible | Application terms cannot express "I am a specialization of a foundation term" — no cross-boundary edges | Two-pass registration with per-vocabulary index injection; collision detection prevents conflicting injections |

##### Key files

- `runtime/src/main/java/io/casehub/eidos/runtime/vocabulary/CdiVocabularyRegistry.java` — `@ApplicationScoped`; discovers `@Any Instance<VocabularyRegistrar>`; six internal maps: byUri/byClass/byClassOrdered/valueToVocabs/ancestorIndex/descendantIndex; two-pass registration (`registerTerms` → `buildAllHierarchyIndexes`); per-vocabulary index injection for cross-vocabulary subsumption; inline collision detection; late `register()` atomicity with snapshot-rollback
...
- `api/src/main/java/io/casehub/eidos/api/VocabularyTerm.java` — interface implemented by vocabulary enum constants; `exactMatch` cross-references (same vocab); `axisExactMatch` cross-references (axis-discriminated, for DISC-as-disposition); `impliesSupervision()` default method for escalation expectation derivation
...
- `api/src/main/java/io/casehub/eidos/api/spi/VocabularyRegistrar.java` — `@FunctionalInterface`; `Class<? extends Enum<? extends VocabularyTerm>> vocabulary()`; `@ApplicationScoped` beans return the vocabulary enum class for auto-registration
...
**`@Any Instance<VocabularyRegistrar>`** discovers all registrar beans. `CdiVocabularyRegistry` iterates at `@PostConstruct` and calls `registerTermsRaw(r.vocabulary())` for each; the registrar returns the vocabulary enum class and the registry handles registration. Do not call `.get()` lazily; the iterator is stateful.

**Two-pass registration** — Pass 1 (`registerTerms`): registers enum constants into `byUri`/`byClass`/`byClassOrdered` maps and populates `valueToVocabs` (reverse index: value → set of vocabulary URIs). Pass 2 (`buildAllHierarchyIndexes`): walks `specializes()` edges across all registered vocabularies to build `ancestorIndex` and `descendantIndex` DAGs; injects cross-vocabulary entries into per-vocabulary indexes for bidirectional `match()`. Two-pass is required because `specializes()` can reference terms from vocabularies registered later in the iteration order.

**Per-vocabulary index injection** — when term A (vocab X) specializes term B (vocab Y), the ancestor/descendant relationship is injected into both vocab X's and vocab Y's indexes. This enables `match("vocabY-uri", "termB", "termA")` to return `Specialization(depth)` even though termA is declared in a different vocabulary. Inline collision detection rejects conflicting injections: native-vs-injected (an injected entry would shadow a term's own hierarchy) and injected-vs-injected (two different vocabularies inject conflicting entries for the same target term).
...
**Late `register()` atomicity** — vocabularies registered after `@PostConstruct` (e.g. from application-tier `@Observes StartupEvent`) are handled atomically: snapshot current index state → register terms → rebuild all hierarchy indexes → on failure, rollback to snapshot. Prevents partial registration from leaving inconsistent ancestor/descendant indexes.
...
**After:** `DefaultCapabilityHealth` — fail-fast 6-step probe (Degraded → Unavailable → Excluded(DECLARED) → Excluded(LEARNED) → EpistemicallyWeak → BehavioralViolation → Ready); `AgentStateStore` SPI records runtime degradation with TTL; `BehavioralSignalStore` SPI accumulates compliance signals; `NoOpAgentStateStore @DefaultBean` and `NoOpBehavioralSignalStore @DefaultBean` preserve backward behaviour.

What this layer adds:

- **Fail-fast probe order (6 steps)** — Step 1: `AgentStateStore.query()` (Degraded); Step 2: capability resolution via `CapabilityResolver.resolve()` (Unavailable if no match); Step 3: declared `excludedDomains` (Excluded/DECLARED); Step 4: learned exclusion via `BehavioralSignalStore.count(DECLINE)` (Excluded/LEARNED); Step 5: epistemic domain confidence (EpistemicallyWeak); Step 6: behavioral compliance via `BehavioralSignalStore.learned(VIOLATED)` — per-dimension check then aggregate total (BehavioralViolation); else Ready
...
- **`BehavioralSignalStore` compliance signals** — `record/clear/learned/count` parameterized by `BehavioralSignal { DECLINE, SUCCESS, COMPLIANT, VIOLATED }`; qualifier parameter (task domain for DECLINE/SUCCESS, `ComplianceDimension` key for COMPLIANT/VIOLATED); per-signal TTL owned by store implementation; `NoOpBehavioralSignalStore @DefaultBean`; `InMemoryBehavioralSignalStore @Alternative @Priority(1)`; `JpaBehavioralSignalStore` in runtime (Flyway V5)
- **Behavioral compliance checking (Step 6)** — queries accumulated VIOLATED signals per capability; per-dimension threshold (default 3, configurable per-tenancy via `PreferenceProvider`); aggregate threshold (default 5) catches cross-dimensional drift when no single dimension exceeds the per-dimension threshold; `BehavioralViolation(violations, ViolationKind)` where `ViolationKind { PER_DIMENSION, AGGREGATE }`
...
- **Six `CapabilityStatus` variants** — `Ready`, `Unavailable`, `Degraded(reason, message)`, `EpistemicallyWeak(domain, confidence)`, `Excluded(domain, ExclusionSource, declineCount)`, `BehavioralViolation(violations, ViolationKind)`; sealed hierarchy; exhaustive switch at call sites
- **`BehavioralExpectations` utility** — `latencyBound(AgentCapability)` → `OptionalLong`, `delegationExpected(AgentDisposition)` → `boolean`, `escalationExpected(AgentDisposition, String, VocabularyRegistry)` → `boolean`; extracts testable expectations from descriptor fields for compliance observation
- **`ComplianceAttestations` utility** — constructs `LedgerAttestation` records from compliance observations for trust scoring impact (attestorId=`eidos:compliance`, trustDimension=`behavioral:<dimension>`)
...
- `eidos-core/src/main/java/io/casehub/eidos/core/health/NoOpAgentStateStore.java` — `@DefaultBean @ApplicationScoped`; all methods no-op; preserves Phase 2 behaviour when no state store is configured
...
| `AgentCapability` | A declared capability with name, qualityHint (0.0–1.0), latencyHintP50Ms, epistemicDomains map (domain → confidence), excludedDomains set (categorical negative specialization), inputTypes, outputTypes, tags; Builder inner class |
...
| `AgentRegistry` / `ReactiveAgentRegistry` | SPI for registering and discovering `AgentDescriptor` instances; `find(AgentQuery)` returns `List<AgentMatch>` carrying descriptor + `ResolvedCapability` (matched capability and OWLS-MX `MatchDegree`); results ordered by match quality when capability is queried |
| `AgentMatch` | Record carrying result of `AgentRegistry.find()`: `AgentDescriptor` + optional `ResolvedCapability` (capability name + OWLS-MX `MatchDegree` from subsumption matching) |
| `ResolvedCapability` | Record carrying result of `CapabilityResolver.resolve()`: `AgentCapability` + `MatchDegree` (Exact, Plugin(depth), Specialization(depth), None); preserves match metadata from vocabulary subsumption resolution |
| `VocabularyRegistry` / `CdiVocabularyRegistry` | SPI + `@ApplicationScoped` CDI implementation; discovers `VocabularyRegistrar` CDI beans; supports `resolve`, `equivalentValues` (cross-vocabulary `exactMatches` traversal), and XKOS-style subsumption hierarchy: `subsumes()`, `match()` → `MatchDegree`, `ancestors()`, `descendants()`, `expandForMatchingByVocabulary()`; two-pass registration builds global DAG across vocabulary boundaries |
| `VocabularyTerm` | Interface implemented by vocabulary enum constants; `exactMatch()` cross-references for equivalence; `axisExactMatch()` for axis-discriminated resolution; `specializes()` declares XKOS-style parent terms for subsumption hierarchy; `impliesSupervision()` default method for escalation expectation derivation |
| `MatchDegree` | Sealed interface returned by `VocabularyRegistry.match()`: `Exact` (same term), `Plugin(depth)` (declared is more specific than requested — declared specializes requested), `Specialization(depth)` (declared is more general than requested — requested specializes declared), `None` (no hierarchy relationship) |
...
| `CapabilityStatus` | Sealed hierarchy: `Ready`, `Unavailable(message)`, `Degraded(reason, message)`, `EpistemicallyWeak(domain, confidence)`, `Excluded(domain, ExclusionSource, declineCount)`, `BehavioralViolation(violations, ViolationKind)` |
| `BehavioralViolation.ViolationKind` | Enum: `PER_DIMENSION` (single dimension exceeds threshold — violations map contains only exceeding dimensions), `AGGREGATE` (cross-dimensional drift — violations map contains all dimensions with violations) |
| `BehavioralSignal` | Enum: `DECLINE`, `SUCCESS`, `COMPLIANT`, `VIOLATED` — discriminator for `BehavioralSignalStore` methods; DECLINE/SUCCESS track learned domain specialization; COMPLIANT/VIOLATED track behavioral compliance |
| `BehavioralSignalStore` | SPI accumulating learned behavioral signals; signal-parameterized methods: `record`, `clear`, `learned`, `count`; qualifier parameter (task domain for DECLINE/SUCCESS, `ComplianceDimension` key for COMPLIANT/VIOLATED); per-signal TTL owned by store implementation |
| `ComplianceDimension` | Constants class: `LATENCY`, `ATTESTATION_RATE`, `DELEGATION`, `ESCALATION` dimension keys; `ATTESTOR_ID` (`eidos:compliance`), `TRUST_DIMENSION_PREFIX` (`behavioral:`), `LATENCY_VIOLATION_MULTIPLIER` (2.0) conventions |
| `BehavioralExpectations` | Static utility: `latencyBound(AgentCapability)` → `OptionalLong`, `delegationExpected(AgentDisposition)` → `boolean`, `escalationExpected(AgentDisposition, String, VocabularyRegistry)` → `boolean`; extracts testable expectations from descriptor fields |
| `ComplianceAttestations` | Static utility: constructs `LedgerAttestation` records from compliance observations; attestorId=`eidos:compliance`, trustDimension=`behavioral:<dimension>` |
| `CapabilityResolver` | Static utility in api/: `resolve(capabilities, tag, registry)` → `ResolvedCapability(capability, degree)` preserving match metadata; `match(capability, tag, registry)` → `MatchDegree`; shared by probe and recording paths; callers of `BehavioralSignalStore.record()` must pass the declared capability name |
