...
- **DisplayTermResolver** — SPI for resolving vocabulary term values to display labels; `resolveLabel(value, sourceVocabUri, targetVocabUri, axis)` with cross-vocabulary swap via `equivalentValues`/`axisExactMatch`; `DefaultDisplayTermResolver` `@DefaultBean` in runtime delegates to `VocabularyRegistry`; auto-discovers source vocabulary when `sourceVocabUri` is null; temporary home before move to platform-api (casehubio/platform#283)
