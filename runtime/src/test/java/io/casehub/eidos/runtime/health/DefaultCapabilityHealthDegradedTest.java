...
class DefaultCapabilityHealthDegradedTest {
    ...
    static class StubStateStore implements AgentStateStore {
        ...
        public Optional<DegradationReason> query(final String agentId) {
            return Optional.ofNullable(state.get(agentId));
        }
    ...
    void degraded_state_takes_precedence_over_ready() {
        stateStore.record("agent-1", DegradationReason.RATE_LIMITED, Instant.now().plusSeconds(60));
    ...
    void degraded_state_takes_precedence_over_epistemically_weak() {
        stateStore.record("agent-1", DegradationReason.OVERLOADED, Instant.now().plusSeconds(60));
    ...
    void degraded_detail_identifies_reason() {
        stateStore.record("agent-2", DegradationReason.CONTEXT_EXHAUSTED, Instant.now().plusSeconds(60));
