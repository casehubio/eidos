...
class InMemoryDispositionSignalStoreTest {
    ...
    @Test
    void decay_removes_fraction_of_counts() {
    ...
    @Test
    void decay_preserves_higher_counts() {
        for (int i = 0; i < 100; i++) store.recordActivation("a", "t", "fi");
        store.decay("a", "t", 0.20);
        assertThat(store.activationCounts("a", "t")).containsEntry("fi", 80);
    }
    ...
    void decay_truncates_to_integer() {
        ...
        store.decay("a", "t", 0.20);
        // 3 * 0.8 = 2.4 → truncated to 2
        assertThat(store.activationCounts("a", "t")).containsEntry("se", 2);
    }

    @Test
    void decay_zero_factor_retains_all() {
    ...
    @Test
    void decay_full_factor_clears_all() {
        for (int i = 0; i < 5; i++) store.recordActivation("a", "t", "ti");
        store.decay("a", "t", 1.0);
