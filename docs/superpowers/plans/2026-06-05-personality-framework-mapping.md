# Personality Framework Mapping Document — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Write `docs/personality-frameworks.md` — the reference and design guide that grounds the eidos vocabulary system in established personality, team-role, and occupational frameworks, and makes `BelbinVocabularyProducer` implementation mechanical for eidos#26.

**Architecture:** Single markdown file, two parts. Part I is encyclopedic (11 frameworks by category, each with a mapping table to AgentDescriptor fields). Part II is opinionated (encoding guidance, combination patterns, vocabulary draft tables normative for #26). Content is driven entirely by the approved spec at `docs/superpowers/specs/2026-06-05-personality-framework-mapping-design.md`.

**Tech Stack:** Markdown. No code changes. Verify by reading the file and checking completeness against the spec.

**Spec reference:** `docs/superpowers/specs/2026-06-05-personality-framework-mapping-design.md`

---

### Task 1: File skeleton and Part I Preamble

**Files:**
- Create: `docs/personality-frameworks.md`

- [ ] **Step 1: Write the file skeleton and preamble**

Write `docs/personality-frameworks.md` with this exact opening:
...
- **DISC:** Included as a disposition vocabulary source despite Low scientific validity —
  ...
  extension — see eidos#40 before writing `DiscVocabularyProducer`.
...
Check: terms table lists all 12 Conscientiousness vocabulary keys. MBTI, DISC, and SL
preamble notes are present. eidos#40 warning on DISC. File created at
`docs/personality-frameworks.md`.

---

### Task 2: Part I — Team Role frameworks

**Files:**
- Modify: `docs/personality-frameworks.md`

- [ ] **Step 1: Write the Team Role section**

Append to `docs/personality-frameworks.md`:
...
| Completer Finisher | `completer-finisher` | `independent` | `strict` | `conservative` | `directed` | `false` |
...
- [ ] **Step 2: Verify Team Role section**

Check: Belbin has all 9 roles, all 5 disposition columns populated, delegation column present.
MM has compare/contrast table against all 8 roles. Both have all four required fields
(what it models, validity, adoption, vocabulary role). Note present clarifying disposition
columns are annotation only.

---

### Task 3: Part I — Personality/Behavioral frameworks

**Files:**
- Modify: `docs/personality-frameworks.md`

- [ ] **Step 1: Write Big Five entry**
...
⚠ **Implementation blocked on eidos#40** — the current `VocabularyRegistry.equivalentValues()`
signature cannot resolve a DISC type to different Conscientiousness terms per axis.
...
Check: Big Five has all 5 dimensions (high/low where applicable). DISC has all 4 types,
no delegation column, eidos#40 warning present. MBTI has all 4 dichotomies, "reference
only" vocabulary role, no recommendation for vocabulary module. All three have the four
required fields.

---

### Task 4: Part I — Cognitive/Work Style and Occupational frameworks

**Files:**
- Modify: `docs/personality-frameworks.md`

- [ ] **Step 1: Write Cognitive/Work Style section**
...
|-----------|---------------|--------------|
| Adaptor (high KAI score) | `strict` | `conservative` |
| Innovator (low KAI score) | `flexible` | `bold` |
...
- [ ] **Step 2: Write Occupational frameworks section**
...
~100 skills across six categories and seven responsibility levels (1 = Follow to
...
- [ ] **Step 3: Verify Cognitive/Work Style and Occupational sections**

Check: TK has all 5 modes, eidos#38 reference, "reference only" role. SL has table with
all 4 styles, clear note that this is leader behavior not agent trait, validity=Low.
KAI has Adaptor/Innovator rows, only ruleFollowing and riskAppetite columns. O*NET has
all 6 components. SFIA has responsibility level mapping and IT-domain note.

---

### Task 5: Part I — Cross-Reference Table and Framework Compatibility

**Files:**
- Modify: `docs/personality-frameworks.md`

- [ ] **Step 1: Write cross-reference summary table**
...
| `socialOrient` | **disposition** | reference | partial | **disposition** | partial | partial | — | — | — | — |
| `ruleFollowing` | **disposition** | reference | partial | **disposition** | partial | — | — | partial | — | — |
| `riskAppetite` | **disposition** | reference | partial | **disposition** | — | — | — | partial | — | — |
| `autonomy` | **disposition** | reference | — | **disposition** | — | — | reference | — | — | partial |
| `delegation` | **disposition** | — | — | — | — | — | — | — | — | — |

Key: **bold** = primary vocabulary source for this field · `partial` = partial/approximate mapping ·
`reference` = conceptual grounding only, no vocabulary terms · `—` = no claim
...
- [ ] **Step 2: Verify cross-reference and compatibility**

Check: Table has 7 rows (all AgentDescriptor disposition fields + slot + capabilities),
10 framework columns, BDI excluded with note in heading. Compatibility table has all
10 pairs from the spec, Belbin+DISC note mentions eidos#40.

---

### Task 6: Part II — Architecture Note, URI Interaction, Encoding Guidance

**Files:**
- Modify: `docs/personality-frameworks.md`

- [ ] **Step 1: Write DISC architecture note and vocabulary URI interaction**
...
also a D-type (DISC: assertive, bold) diverges on `socialOrient` and `riskAppetite`.
The DISC type reveals behavioral style that the Belbin role does not predict.

⚠ **API gap — eidos#40:** `VocabularyRegistry.equivalentValues(fromVocab, value, toVocab)`
has no axis parameter. A DISC type maps to *different* Conscientiousness terms on each
axis (`dominance → independent` on socialOrient, `bold` on riskAppetite, etc.). The
current signature cannot disambiguate. Do not implement `DiscVocabularyProducer` until
eidos#40 is resolved.
...
- [ ] **Step 2: Verify architecture and encoding sections**

Check: DISC architecture note has both Belbin and DISC vocabulary explained with distinction,
eidos#40 warning. Three worked examples present (Belbin Profile, Belbin+DISC, Occupational).
Encoding guidance has all 4 rules. co-ordinator used in Belbin+DISC example (not shaper).
delegation=true in Belbin+DISC example with note about Belbin source.

---

### Task 7: Part II — Axis Recommendations, Combination Patterns, Anti-Patterns

**Files:**
- Modify: `docs/personality-frameworks.md`

- [ ] **Step 1: Write axis recommendations**
...
— e.g., a Co-ordinator (facilitative, measured) who is also a D-type (assertive, bold).
...
`socialOrient="shaper"` treats a slot vocabulary key as a disposition term. This is a
...
**6. Thomas-Kilmann Competing → `riskAppetite`**
...
- [ ] **Step 3: Verify axis recommendations, combination patterns, anti-patterns**

Check: All 7 AgentDescriptor fields have a recommendation. Three combination patterns
present with full code blocks and what-it-expresses/leaves-unspecified/when-to-use.
Belbin+DISC shows co-ordinator (not shaper), delegation from Belbin not DISC. Anti-patterns
grouped into three sections (7 total). Pattern 4 and 5 are in Encoding Errors not mixed
with selection or axis errors.

---

### Task 8: Part II — Vocabulary Draft Tables, Gap Notes, Implementation Notes, BDI Appendix

**Files:**
- Modify: `docs/personality-frameworks.md`

- [ ] **Step 1: Write vocabulary draft tables**
...
These tables are the normative source for eidos#26 implementation. The implementor
writes one `Map.entry(key, new VocabularyTerm(...))` per row.
...
**Implementation note:** The disposition columns below (`socialOrient`, `ruleFollowing`,
`riskAppetite`, `autonomy`, `delegation`) are reference annotation showing values implied
by each role. They are **not** fields in `VocabularyTerm`. `BelbinVocabularyProducer`
writes only: `value`, `label`, `description`, `aliases`, `exactMatches = Map.of()`.
...
| `completer-finisher` | `independent` | `strict` | `conservative` | `directed` | `false` |
...
**⚠ Implementation blocked on eidos#40.** Resolve the axis-aware resolution mechanism
before writing `DiscVocabularyProducer`. The `→` columns below show the Conscientiousness
term that must be returned when resolving this DISC type on each specific axis — this is
the mapping the producer must implement, but the current `equivalentValues()` signature
cannot express it.

| DISC type | value (key) | label | description | aliases | → socialOrient | → ruleFollowing | → riskAppetite | → autonomy |
|-----------|-------------|-------|-------------|---------|----------------|-----------------|----------------|------------|
...
Note: `delegation` is absent — DISC types make no claim about sub-agent spawning.
The key `conscientiousness-disc` avoids collision with `urn:casehub:vocab:conscientiousness`
...
|-----|--------|-----------------|------|
| Co-ordinator autonomy | Belbin Co-ordinator | Orchestrates team decision-making; higher coordination intent than `semi-autonomous` but not independently agenda-driven like `autonomous` | Mapped to `semi-autonomous + delegation=true` — the combination may be sufficient; decision for #26 |
...
- Keep Belbin and DISC as separate producer classes (`BelbinVocabularyProducer`, `DiscVocabularyProducer`) in the same `casehub-eidos-vocab` module
- **`DiscVocabularyProducer` is blocked on eidos#40** — resolve the axis-aware resolution mechanism before writing it

---
```

- [ ] **Step 2: Write BDI appendix**
...
- [ ] **Step 3: Verify vocabulary tables and appendix**

Check: Belbin VocabularyTerm table has 5 columns (value, label, description, aliases) —
no disposition columns in the term table. Separate implied-disposition reference table
present. DISC table has eidos#40 warning, 4 DISC types, no delegation column.
Gap notes table has 2 rows. Implementation notes list all constructor signatures.
BDI appendix has "illustrative, not a precise correspondence" qualifier, all 3 BDI
components, and the analogy-break column.
...
**Files:**
- Commit: `docs/personality-frameworks.md`

- [ ] **Step 1: Verify file completeness against spec**

Check that `docs/personality-frameworks.md` contains all required sections:

```
Part I:
  [ ] Preamble (how to read tables, notes on MBTI/DISC/SL)
  [ ] 1. Team Role: Belbin (9 roles, table), Margerison-McCann (compare/contrast)
  [ ] 2. Personality/Behavioral: Big Five, DISC (eidos#40 warning), MBTI
  [ ] 3. Cognitive/Work Style: TK, SL, KAI
  [ ] 4. Occupational: O*NET, SFIA
  [ ] 5. Cross-reference summary table (7 rows × 10 columns)
  [ ] 6. Framework compatibility (10 pairs)

Part II:
  [ ] Architecture: DISC as disposition vocabulary, eidos#40 warning
  [ ] Vocabulary URI field interaction with 3 worked examples
  [ ] Encoding guidance (4 rules)
  [ ] Axis-by-axis recommendations (7 fields)
  [ ] 3 combination patterns (Belbin, Belbin+DISC, Occupational)
  [ ] Anti-patterns (7, in 3 grouped sections)
  [ ] Vocabulary draft tables: Belbin (VocabularyTerm table + implied-disposition table), DISC
  [ ] Vocabulary gap notes (2 rows)
...
git -C /Users/mdproctor/claude/casehub/eidos commit -m "docs: personality framework mapping — Part I reference + Part II design guide

Part I: 11 frameworks across 5 categories with AgentDescriptor mapping tables.
Part II: encoding guidance, 3 combination patterns, 7 anti-patterns, normative
vocabulary draft tables for eidos#26 (Belbin mechanical; DISC blocked on eidos#40).
...
```bash
git -C /Users/mdproctor/claude/casehub/eidos push
```
