...
|---|---|---|---|
| `api/` | `casehub-eidos-api` | Pure Java + `platform-api` | SPIs and domain types -- `AgentDescriptor`, `AgentRegistry`, `CapabilityHealth`, `VocabularyRegistry`, `SystemPromptRenderer`, `AgentStateStore`, `BehavioralSignalStore`, `DispositionHealth`, `DispositionEvolution`, `DispositionSignalStore`, `RenderedPromptCache`, `TemplateRegistry`, `AgentSelector`, `DisplayTermResolver`; sealed types: `MatchDegree` (Exact, Plugin, Specialization, None), `CapabilityStatus`, `AgentSelection`, `DispositionStatus`, `EvolutionResult`; utilities: `CapabilityResolver`, `BehavioralExpectations`, `AgentDescriptorComparator`, `ExtensionDataCopier`, `PersonalityTypeDeriver`; template types: `DescriptorTemplate`, `TemplateRef`; SPI package (`api.spi`): `AgentDescriptorRegistrar`, `VocabularyRegistrar`, `TemplateRegistrar`; graph SPIs: `AgentGraphStore`, `AgentGraphQuery`, `AgentGraphBackfill`, `TaskSemanticEnricher`. Depends on `casehub-platform-api` for `ModelQuery` (model selection on `AgentCapability`) |
| `eidos-core/` | `casehub-eidos-core` | Pure CDI, no Quarkus | Framework-neutral implementations extracted from runtime (#169). Subpackages: `yaml/` (`EidosDescriptorModule`, `AgentDescriptorDeserializer`, `DescriptorPreprocessor`, `DescriptorForEachAdapter`), `renderer/` (`EidosRenderPipeline`, enrichment steps, `JsonExtractionUtil`), `display/` (`DefaultDisplayTermResolver`), `health/` (NoOp defaults: `NoOpAgentStateStore`, `NoOpBehavioralSignalStore`, `NoOpDispositionSignalStore`, `NoOpDispositionHealth`, `NoOpDispositionEvolution`, `NoOpCapabilityHealth`), `preferences/` (preference keys and defaults), `registrar/` (`DescriptorCollector` validation), `validator/` (`CapabilityVocabularyValidator` runtime), `template/` (`ClasspathYamlTemplateRegistrar`), `graph/` (NoOp graph implementations) |
| `runtime/` | `casehub-eidos` | Quarkus extension | CDI registry, JPA persistence, Flyway migrations, Quarkus-specific wiring. Subpackages: `registry/jpa/` (JPA registries, entity classes, mapper, `ModelQueryConverter`), `vocabulary/` (`CdiVocabularyRegistry`), `health/` (JPA-backed signal stores, compliance attestations), `registrar/` (`AgentDescriptorBootstrap`, reactive bootstrap, `ArchetypeDeriver`), `selector/` (`SimpleAgentSelector`), `template/` (`CdiTemplateRegistry`), `renderer/` (`EidosSystemPromptRenderer` — wraps eidos-core pipeline) |
| `annotations/` | `casehub-eidos-annotations` | Quarkus extension (nested) | Annotation-driven agent identity: `@Identity`, `@Disposition`, `@AgentGoals`, `@AgentConstraints`, `@AgentCapabilityDef` (repeatable, with `model` string shorthand + `modelTier`/`modelCapabilities` structured form), `@AgentTemplateRef`, `@ExtensionData`. Build-time processor generates `AgentDescriptorRegistrar` beans. Hybrid vocabulary validation at build time. Nested parent with `runtime/` and `deployment/` submodules |
...
| `vocab/` | `casehub-eidos-vocab` | Optional module | Well-known vocabularies: `SvoTerm`, `ConscientiousnessTerm`, `CasehubSlotTerm`, `BelbinTerm` (9 Belbin team roles), `DiscTerm` (4 DISC types), `ThomasKilmannTerm` (5 conflict modes), `CasehubCapabilityTerm` (hierarchical capability taxonomy), `JungianFunctionTerm` (8 cognitive functions), `MbtiTypeTerm` (16 MBTI types), `JungianEvolutionType` (4 JPAF reflection types), `BigFiveTerm`, `EnneagramTerm`, `SdiTerm`, `ModelTierTerm` (4 LLM model tiers with linear subsumption), `ArchetypeTerm` (48 sub-archetypes in 12 families), `ArchetypeFamily`, `ArchetypeCompatibility` (6-framework mapping), `ArchetypeResolver` (set-intersection derivation). Each vocabulary enum accompanied by a `VocabularyRegistrar` bean |
| `routing/` | `casehub-eidos-routing` | Optional module | Engine-aware agent selection bridge: `EngineAwareAgentSelector` `@Alternative @Priority(1)` converts `AgentMatch` → `AgentCandidate`, delegates to engine's `AgentRoutingStrategy`. Depends on `eidos-api` + `engine-api` + `ledger-api` |
...
| `org-api/` | `casehub-eidos-org-api` | Pure Java | Organizational model POJOs: `OrganizationalUnit`, `AgentRelationship`, `Membership`, `RelationshipKind`, `RelationshipScope`, `AttestationGrant`, `OrgQuery`, `OrgRegistry` SPI, `OrgRegistrar` SPI, `OrgStructure` compositional DSL |
| `org-runtime/` | `casehub-eidos-org` | Quarkus extension | `OrgBootstrap` (startup registrar discovery), `OrgGoalCompiler` (desiredstate integration), `ClasspathYamlOrgRegistrar` (META-INF/eidos/organization.yaml), `EidosOrgModule` (Jackson) |
| `org-memory/` | `casehub-eidos-org-memory` | Optional module | `@Alternative @Priority(1)` `InMemoryOrgRegistry` with cycle detection |
| `org-annotations/` | `casehub-eidos-org-annotations` | Quarkus extension (nested) | Annotation-driven org structure: `@OrgUnit`, `@OrgMembers`, `@Supervises` (repeatable), `@OrgRelationships`. Build-time processor generates `OrgRegistrar` beans |
| `eval/` | `casehub-eidos-eval` | Test-only, not deployed | Offline quality evaluation harness (see Eval Harness section below) |
| `examples/` | -- | Test-only | `@QuarkusTest` integration scenarios: `agent-scenarios/` (team, cross-vocab, epistemic, tenancy, disposition, model selection), `org-scenarios/` (hierarchy, review team, customer support, clinical triage, capability matrix, YAML org), plus standalone agent examples (code-review, creative-director, medical-scribe, customer-support, child-companion, tutor) |
