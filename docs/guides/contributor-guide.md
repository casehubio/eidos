...
|---|---|---|---|
| `api/` | `casehub-eidos-api` | Pure Java, no CDI | SPIs and domain types -- `AgentDescriptor`, `AgentRegistry`, `CapabilityHealth`, `VocabularyRegistry`, `SystemPromptRenderer`, `AgentStateStore`, `BehavioralSignalStore`, `DispositionHealth`, `DispositionEvolution`, `DispositionSignalStore`, `RenderedPromptCache`, `TemplateRegistry`; sealed types: `MatchDegree` (Exact, Plugin, Specialization, None), `CapabilityStatus`, `DispositionStatus`, `EvolutionResult`; utilities: `CapabilityResolver`, `BehavioralExpectations`, `AgentDescriptorComparator`, `ExtensionDataCopier`; template types: `DescriptorTemplate`, `TemplateRef`; SPI package (`api.spi`): `AgentDescriptorRegistrar`, `VocabularyRegistrar`, `TemplateRegistrar`; graph SPIs (also in api/): `AgentGraphStore`, `AgentGraphQuery`, `AgentGraphBackfill`, `TaskSemanticEnricher` |
