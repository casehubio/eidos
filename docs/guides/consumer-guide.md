...
- `modelTier` (String, optional) -- vocabulary-grounded LLM model tier requirement (e.g. `"flagship"`, `"standard"`); validated against `urn:casehub:vocab:model-tier` when the vocabulary is registered. Platform reads this at dispatch time to resolve a concrete model.
- `modelCapabilities` (Set<String>, optional) -- required model vendor features (e.g. `"text"`, `"vision"`, `"tool-use"`); open strings, no vocabulary grounding. Pass-through filter for the platform router.
...
| `A2A_CARD` | Machine-readable agent card (JSON) | Full routing signals: `qualityHint`, `latencyHintP50Ms`, `costHint`, `epistemicDomains`, `modelTier`, `modelCapabilities`, `inputTypes`/`outputTypes`, `excludedDomains`. Slot and disposition with vocabulary context. `frameworks` array of actively-instantiated vocabulary URIs. |
